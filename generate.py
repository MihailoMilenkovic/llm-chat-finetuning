...
from consts import END_KEY, PROMPT_FORMAT_BEFORE_RESPONSE, RESPONSE_KEY, DEFAULT_INPUT_MODEL, DEFAULT_MODEL_PATH
...
def load_model_tokenizer_for_generate(
    ...
    model = AutoModelForCausalLM.from_pretrained(
        pretrained_model_name_or_path, device_map="auto", torch_dtype=torch.float16, trust_remote_code=True
    )
    if adapter_location!=None:
        model=PeftModel.from_pretrained(model,adapter_location,torch_dtype=torch.float16)
...
class InstructionTextGenerationPipeline(Pipeline):
    ...
    def preprocess(self, instruction_text, **generate_kwargs):
        prompt_text = PROMPT_FORMAT_BEFORE_RESPONSE.format(instruction=instruction_text,context="")
